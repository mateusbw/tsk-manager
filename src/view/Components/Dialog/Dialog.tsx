...
const Background = styled.div<BackgroundProps>`
    ...
    min-height: 100vh;
    overflow: hidden;
    background-color: ${(props) => transparentize(0.1, props.theme.colors.gray[950])};
    ${(props) => props.isOpened ? 
    css`animation: ${fadeIn} 0.5s forwards;` :  
    css`animation: ${fadeOut} 0.5s forwards;`
}
`
    ...
    return (
        <Background isOpened={isOpened}>
            <BaseDialog isOpened={isOpened}>
                <CloseButton callBack={() => onClose()} variant="icon"><CloseIcon/></CloseButton>
                {children}
            </BaseDialog>
        </Background>
    )
